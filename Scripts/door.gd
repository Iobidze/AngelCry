...
@export var angel_sound: AudioStreamPlayer2D
...
func _on_angel_entered(body):
	if body.is_in_group("Angel"):
		...
		# Проигрываем звук сразу при входе ангела в зону
		play_angel_sound()
		
		print("Ангел приблизился к двери и звук проигран!")
...
func _on_angel_exited(body):
	if body.is_in_group("Angel"):
		...
		angel_timer = 0.0
		angel_triggered = false
		print("Ангел отошёл от двери")

func play_angel_sound():
	if angel_sound:
		angel_sound.play()
		print("Звук ангела проигрывается!")
	else:
		print("Звук не назначен! Добавь AudioStreamPlayer2D в экспортную переменную")
